...
def dopdp(infolder, outname=None, window_size=1,max_workers=None):
    input_filepaths = [
    ...
    if outname is None:
        outname = os.path.join(infolder, "dopdp.tif")
    process_chunks_parallel(outname, input_filepaths, processing_func=process_chunk_dopdp, window_size=window_size, max_workers=max_workers)

def process_chunk_dopdp(chunks, window_size):
