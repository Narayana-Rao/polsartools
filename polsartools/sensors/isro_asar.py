...
from polsartools.utils.geo_utils import geocode, intp_grid, update_vrt, write_latlon
...
@time_it
def isro_asar(inFile,matrix='T3',azlks=8,rglks=6,calibration_constant = 42):
    ...
    if matrix == 'S2':
        ...
        def write_element(data,outname):
            ...
            update_vrt('data.vrt')
            input_vrt = "data.vrt"
            geocode(input_vrt, outname, 'bin', x_res_deg, y_res_deg, bbox,resampleAlg="near")
            os.remove('data.vrt')
            os.remove('data.bin')
            os.remove('data.hdr')
